...
// ---------- Уборка диска ----------
// Фотографии и видео удалённых моторов остаются на диске навсегда: карточку
// убрали, а папка с роликами весит гигабайты. Когда диск заполняется, сервер
// перестаёт создавать новые файлы — и панель молча перестаёт сохранять всё
// подряд, потому что запись идёт через временный файл.
//
// Эти два действия показывают, что лежит лишнего, и убирают это по кнопке.
// Лишним считается только то, на что не ссылается ни каталог, ни архив
// проданных: снятый с продажи мотор можно вернуть, и его видео должны уцелеть.

function media_root(): string
{
    $real = realpath(ROOT_DIR . '/media');
    return $real === false ? '' : $real;
}

// Пути из каталога приходят по-разному: «/media/...», «media/...», иногда
// с параметрами. Приводим к одному виду — путь от корня сайта.
function media_key(?string $url): ?string
{
    ...
    $path = ltrim($path, '/');
    return str_starts_with($path, 'media/') ? $path : null;
}

function referenced_media(): array
{
    $used = [];
    foreach ([load_json_file(MOTORS_FILE), load_json_file(SOLD_FILE)] as $list) {
        foreach ($list as $motor) {
            if (!is_array($motor)) {
                continue;
            }
            $urls = [$motor['img'] ?? null];
            foreach (($motor['photos'] ?? []) as $photo) {
                $urls[] = is_string($photo) ? $photo : null;
            }
            foreach (($motor['videos'] ?? []) as $video) {
                if (is_array($video)) {
                    $urls[] = $video['url'] ?? null;
                    $urls[] = $video['poster'] ?? null;
                }
            }
            foreach ($urls as $url) {
                $key = media_key(is_string($url) ? $url : null);
                if ($key !== null) {
                    $used[$key] = true;
                }
            }
...
function scan_media(): array
{
    $root = media_root();
    $result = ['total' => 0, 'used' => 0, 'orphanSize' => 0, 'orphanCount' => 0, 'orphans' => []];
    if ($root === '' || !is_dir($root)) {
        return $result;
    }
    $used = referenced_media();
    $orphans = [];
    ...
    foreach ($iterator as $file) {
        ...
        $result['total'] += $size;
        $rel = 'media/' . ltrim(substr($file->getPathname(), strlen($root)), '/');
        if (isset($used[$rel])) {
            $result['used'] += $size;
            continue;
        }
        $result['orphanSize'] += $size;
        $result['orphanCount']++;
        $orphans[] = ['path' => $rel, 'size' => $size];
    }
    usort($orphans, static fn(array $a, array $b): int => $b['size'] <=> $a['size']);
    $result['orphans'] = array_slice($orphans, 0, 40);
    return $result;
}

if ($action === 'diskScan') {
    json_out(200, ['disk' => scan_media(), 'free' => (int) (@disk_free_space(ROOT_DIR) ?: 0)]);
}

if ($action === 'diskClean') {
    $root = media_root();
    if ($root === '') {
        json_out(500, ['error' => 'Папка media не найдена']);
    }
    // Список лишнего считаем заново прямо здесь: принимать его от браузера
    // нельзя — подменённый список превратил бы уборку в удаление каталога.
    $used = referenced_media();
    $removed = 0;
    $freed = 0;
    $iterator = new RecursiveIteratorIterator(
    ...
    foreach ($iterator as $file) {
        ...
        $rel = 'media/' . ltrim(substr($full, strlen($root)), '/');
        if (isset($used[$rel])) {
            continue;
        }
        // Последняя защита: удаляем только то, что действительно лежит
        // внутри media, даже если путь пришёл странным.
        $real = realpath($full);
    ...
    json_out(200, ['removed' => $removed, 'freed' => $freed, 'disk' => scan_media(),
        'free' => (int) (@disk_free_space(ROOT_DIR) ?: 0)]);
}
